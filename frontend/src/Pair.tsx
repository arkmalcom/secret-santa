...
function Pair() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const userResponse = await fetch(`${BASE_API_URL}/users/${listId}/${phoneNumber}`, {
        method: 'GET'
      });
      ...
      const pairResponse = await fetch(`${BASE_API_URL}/pairings/${listId}/${userPublicId}`, {
        method: 'GET'
      });

      if (pairResponse.status === 404) {
        const pairCreateResponse = await fetch(`${BASE_API_URL}/pairings/create/${listId}/${userPublicId}`, {
            method: 'POST',
    ...
    <div className="min-h-screen bg-green-600 flex items-center justify-center text-center text-white flex-col space-y-4">
      {pairName ? (
        <div className="flex justify-center text-center items-center bg-green-800 border-2 border-red-600 flex-col space-y-2 p-6">
          ...
          <p className="text-xl mt-2">El nombre de tu angelito es: <span className="text-red-500 font-semibold">{pairName}</span></p>
        </div>
      ) : (
        <div className="w-full max-w-md bg-white rounded shadow-md p-6">
          <h1 className="text-2xl font-bold mb-4 text-center text-black">Ingresa tu número de teléfono</h1>
